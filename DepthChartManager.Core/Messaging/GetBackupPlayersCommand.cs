...
namespace DepthChartManager.Core.Messaging
{
    ...
    public class GetBackupPlayersCommandHandler : IRequestHandler<GetBackupPlayersCommand, CommandResult<IEnumerable<PlayerDto>>>
    {
        ...
        public Task<CommandResult<IEnumerable<PlayerDto>>> Handle(GetBackupPlayersCommand request, CancellationToken cancellationToken)
        {
            try
            {
              var playerId = _sportRepository.GetPlayer(request.GetBackupPlayersDto.LeagueId,
                request.GetBackupPlayersDto.TeamId, request.GetBackupPlayersDto.PlayerName).Id;

              var backupPlayerPositions = _sportRepository.GetBackupPlayerPositions(request.GetBackupPlayersDto.LeagueId,
                  request.GetBackupPlayersDto.TeamId, playerId,
                  request.GetBackupPlayersDto.PositionName);
